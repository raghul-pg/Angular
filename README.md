...
```bash
cd "c:\Users\Raghul\OneDrive\Documents\Angular-main"
git init
git add .
...
Create a repository on GitHub, then add the remote URL:
```bash
...
### 3. Automatic deployment with GitHub Pages
This repo includes a GitHub Actions workflow at `.github/workflows/deploy.yml`.
When you push to the `main` branch, GitHub will:
- install dependencies
- build the Angular app
- publish `placement/dist/placement` to the `gh-pages` branch
